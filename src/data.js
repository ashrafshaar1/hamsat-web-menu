export const menuData = {
  hamstspeciality: {
    title: "Hamsat Speciality | همسات سبيشليتي",
    description: {
      english: "An unexpected but delightful taste.",
      arabic: "طعم غير متوقع لكنه لذيذ."
    },
    items: [
      {
        id: 1,
        englishTitle: "Jalepeno Cheddar Cheese Triangles.",
        arabicTitle: "مثلثات جبنة شيدر بالجالبينو.",
        price: "500,000 LBP",
        englishDescription: "6 pcs of fried breaded cheddar cheese.",
        arabicDescription: "6 قطع من جبن الشيدر المقلي.",
        image: "uploads/123.jfif"
      },
      {
        id: 2,
        englishTitle: "Calamari Rings.",
        arabicTitle: "حلقات كاليماري.",
        price: "500,000 LBP",
        englishDescription: "6 pcs of fried Breaded Calamari Rings.",
        arabicDescription: "6 قطع من حلقات الكاليماري المقلية.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 3,
        englishTitle: "Herbal Lebanese Wedges.",
        arabicTitle: "ودجز لبنانية بالأعشاب.",
        price: "400,000 LBP",
        englishDescription: "Basil, Thyme, Peprica, Garlic.",
        arabicDescription: "ريحان، زعتر، فلفل حلو، ثوم.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 4,
        englishTitle: "Wings Provincial.",
        arabicTitle: "جوانح.",
        price: "500,000 LBP",
        englishDescription: "Parsly, Corriander, Lemon, Garlic.",
        arabicDescription: "بقدونس، كزبرة، ليمون، ثوم.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 5,
        englishTitle: "Oriental Beef Strips Fillets.",
        arabicTitle: "شرائح لحم بقري شرقية.",
        price: "500,000 LBP",
        englishDescription: "Thyme, Rosemary, Raspberry/Cherry Vinegar.",
        arabicDescription: "زعتر، اكليل الجبل، خل التوت/الكرز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 6,
        englishTitle: "Oriental Chicken Strips.",
        arabicTitle: "شرائح الدجاج الشرقية.",
        price: "500,000 LBP",
        englishDescription: "Grilled Chicken served with Spicy Mayo Dip.",
        arabicDescription: "دجاج مشوي يقدم مع صلصة المايونيز الحارة.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 7,
        englishTitle: "Shrimps Provincial.",
        arabicTitle: "قريدس.",
        price: "600,000 LBP",
        englishDescription: "Shrimps, Parsly, Corriander, Lemon, Garlic.",
        arabicDescription: "قريدس، بقدونس، كزبرة، ليمون، ثوم.",
        image: "uploads/arancini.jpg"
      }
    ]
  },
  starters: {
    title: "Starters | مقبلات",
    description: {
      english: "The beginning of an unforgettable dining saga.",
      arabic: "بداية ملحمة طعام لا تُنسى."
    },
    items: [
      {
        id: 1,
        englishTitle: "Jalepeno Cheddar Cheese Triangles.",
        arabicTitle: "مثلثات جبنة شيدر بالجالبينو.",
        price: "500,000 LBP",
        englishDescription: "6 pcs of fried breaded cheddar cheese.",
        arabicDescription: "6 قطع من جبن الشيدر المقلي.",
        image: "uploads/123.jfif"
      },
      {
        id: 2,
        englishTitle: "Calamari Rings.",
        arabicTitle: "حلقات كاليماري.",
        price: "500,000 LBP",
        englishDescription: "6 pcs of fried Breaded Calamari Rings.",
        arabicDescription: "6 قطع من حلقات الكاليماري المقلية.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 3,
        englishTitle: "Herbal Lebanese Wedges.",
        arabicTitle: "ودجز لبنانية بالأعشاب.",
        price: "400,000 LBP",
        englishDescription: "Basil, Thyme, Peprica, Garlic.",
        arabicDescription: "ريحان، زعتر، فلفل حلو، ثوم.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 4,
        englishTitle: "Wings Provincial.",
        arabicTitle: "جوانح.",
        price: "500,000 LBP",
        englishDescription: "Parsly, Corriander, Lemon, Garlic.",
        arabicDescription: "بقدونس، كزبرة، ليمون، ثوم.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 5,
        englishTitle: "Oriental Beef Strips Fillets.",
        arabicTitle: "شرائح لحم بقري شرقية.",
        price: "500,000 LBP",
        englishDescription: "Thyme, Rosemary, Raspberry/Cherry Vinegar.",
        arabicDescription: "زعتر، اكليل الجبل، خل التوت/الكرز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 6,
        englishTitle: "Oriental Chicken Strips.",
        arabicTitle: "شرائح الدجاج الشرقية.",
        price: "500,000 LBP",
        englishDescription: "Grilled Chicken served with Spicy Mayo Dip.",
        arabicDescription: "دجاج مشوي يقدم مع صلصة المايونيز الحارة.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 7,
        englishTitle: "Shrimps Provincial.",
        arabicTitle: "قريدس.",
        price: "600,000 LBP",
        englishDescription: "Shrimps, Parsly, Corriander, Lemon, Garlic.",
        arabicDescription: "قريدس، بقدونس، كزبرة، ليمون، ثوم.",
        image: "uploads/arancini.jpg"
      }
    ]
  },
  salads: {
    title: "Salads | السلطات",
    description: {
      english: "A treasure trove of crisp, lush ingredients.",
      arabic: "كنز من المكونات اللذيذة والرائعة."
    },
    items: [
      {
        id: 8,
        englishTitle: "Crab Salad.",
        arabicTitle: "سلطة الكراب.",
        price: "500,000 LBP",
        englishDescription: "Mixed Greens (Mayo, Soy Sauce, Sesame Oil).",
        arabicDescription: "خضراوات مشكلة (مايونيز، صلصة الصويا، زيت السمسم).",
        image: "uploads/filet.jpg"
      },
      {
        id: 9,
        englishTitle: "Smoked Salamon Salad.",
        arabicTitle: "سلطة السلمون المدخن.",
        price: "700,000 LBP",
        englishDescription: "Mixed Greens, Smoked Salamon, served with Creamy Lemon Caper Dressing.",
        arabicDescription: "خضراوات مشكلة، سلمون مدخن، يقدم مع صلصة الليمون الكريمية بالكبر.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 10,
        englishTitle: "Shrimp Salad.",
        arabicTitle: "سلطة القريدس.",
        price: "600,000 LBP",
        englishDescription: "Mixed Greens, Shrimps served with Herbal, Olive Oil, Vinaigrette Dressing.",
        arabicDescription: "خضراوات مشكلة، روبيان يقدم مع أعشاب، زيت زيتون، صلصة الخل.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 11,
        englishTitle: "Chicken Strip Salad.",
        arabicTitle: "سلطة شرائح الدجاج.",
        price: "500,000 LBP",
        englishDescription: "Mixed Greens, Grilled Chicken Strips, Creamy Mayo Dressing.",
        arabicDescription: "خضراوات مشكلة، شرائح دجاج مشوية، صلصة مايونيز كريمية.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 12,
        englishTitle: "Chicken Ceaser Salad.",
        arabicTitle: "سلطة سيزر بالدجاج.",
        price: "500,000 LBP",
        englishDescription: "Iceberg Letous, Grilled Chicken Strips, Ceaser Dressing.",
        arabicDescription: "خس آيسبرغ، شرائح دجاج مشوية، صلصة سيزر.",
        image: "uploads/salmon.jpg"
      }
    ]
  },
  opensandwiches: {
    title: "Open Sandwiches | ساندويتش مفتوحة",
    description: {
      english: "Where sandwiches becomes a masterpiece.",
      arabic: "حيث تصبح السندويشات تحفة فنية."
    },
    items: [
      {
        id: 20,
        englishTitle: "Chicken Ceaser Sandwiche.",
        arabicTitle: "ساندويتش دجاج سيزر.",
        price: "500,000 LBP",
        englishDescription: "Home made Sour Dough Bread served with Marinara Sauce, Mixed Cheese, Wedges and Creamy Mayo.",
        arabicDescription: "خبز العجين المخمر محلي الصنع يقدم مع صلصة مارينارا، جبنة مشكلة، ودجز و مايونيز كريمي.",
        image: "uploads/filet.jpg"
      },
      {
        id: 21,
        englishTitle: "Miltreanean Beef Sandwiche.",
        arabicTitle: "ساندويتش لحم البقر الميلترياني.",
        price: "600,000 LBP",
        englishDescription: "Home made Sour Dough Bread served with Marinara Sauce, Mixed Cheese, Wedges and Creamy Pesto.",
        arabicDescription: "خبز العجين المخمر محلي الصنع يقدم مع صلصة مارينارا، جبنة مشكلة، ودج و بيستو كريمي.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 22,
        englishTitle: "Mixed Sea Food Sandwiche.",
        arabicTitle: "ساندويتش مأكولات بحرية مشكلة.",
        price: "650,000 LBP",
        englishDescription: "Home made Sour Dough Bread served with Marinara Sauce, Mixed Cheese, Wedges and Spicey Cocktail.",
        arabicDescription: "خبز العجين المخمر محلي الصنع يقدم مع صلصة مارينارا، جبنة مشكلة، ودجز و كوكتيل حار.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 23,
        englishTitle: "Chicken Pesto Sandwiche.",
        arabicTitle: "ساندويتش دجاج بيستو.",
        price: "500,000 LBP",
        englishDescription: "Home made Sour Dough Bread served with Marinara Sauce, Mixed Cheese, Wedges and Creamy Pesto.",
        arabicDescription: "خبز العجين المخمر محلي الصنع يقدم مع صلصة مارينارا، جبنة مشكلة، ودجز، وبيستو كريمي.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 24,
        englishTitle: "Dinamit Shrimp Sandwiche.",
        arabicTitle: "ساندويتش قريدس ديناميت.",
        price: "650,000 LBP",
        englishDescription: "Homemade Sour Dough Bread served with Marinara Sauce, Mixed Cheese, Wedges and Spicey Cocktail.",
        arabicDescription: "خبز العجين المخمر محلي الصنع يقدم مع صلصة مارينارا، جبنة مشكلة، ودجز و كوكتيل حار.",
        image: "uploads/salmon.jpg"
      },
      {
        id: 25,
        englishTitle: "Mixed Cheese Sandwiche.",
        arabicTitle: "ساندويتش الجبنة المشكلة.",
        price: "450,000 LBP",
        englishDescription: "Homemade Sour Dough Bread served with Marinara Sauce, Mixed Cheese, Wedges and Spicey Cocktail.",
        arabicDescription: "خبز العجين المخمر محلي الصنع يقدم مع صلصة مارينارا، جبنة مشكلة، ودجز و كوكتيل حار.",
        image: "uploads/salmon.jpg"
      }
    ]
  },
  maincourse: {
    title: "Main Courses | الأطباق الرئيسية",
    description: {
      english: "Mighty dishes worthy of warriors and kings.",
      arabic: "أطباق عظيمة تليق بالمحاربين والملوك."
    },
    items: [
      {
        id: 13,
        englishTitle: "Grilled Chicken Breast.",
        arabicTitle: "صدور دجاج مشوية.",
        price: "750,000 LBP",
        englishDescription: "Served with Wedges, Creamy Cajen Sauce.",
        arabicDescription: "يُقدم مع ودجز وصلصة كاجين الكريمية.",
        image: "uploads/bruschetta.jpg"
      },
      {
        id: 14,
        englishTitle: "Grilled Beef Strip.",
        arabicTitle: "شرائح لحم البقر المشوية.",
        price: "850,000 LBP",
        englishDescription: "Served with Creamy Pepper Sauce, Wedges.",
        arabicDescription: "تقدم مع صلصة الفلفل الكريمية و ودجز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 15,
        englishTitle: "Sea Food Mix Platter Fish.",
        arabicTitle: "طبق مزيج المأكولات البحرية والأسماك.",
        price: "650,000 LBP",
        englishDescription: "Deep Fried Crispy Shrimps, Fish, Calamari served with Crab Salad and Wedges.",
        arabicDescription: "قريدس مقرمش مقلي، سمك، كاليماري يقدم مع سلطة الكراب و ودجز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 16,
        englishTitle: "Shrimp Platter (Spicey).",
        arabicTitle: "طبق الروبيان (حار).",
        price: "650,000 LBP",
        englishDescription: "Deep Fried Crispy Shrimps, Tartare, served with Wedges.",
        arabicDescription: "قريدس مقرمش مقلي، طرطور، يقدم مع ودجز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 17,
        englishTitle: "Fish and Chips Platter.",
        arabicTitle: "طبق السمك والبطاطس المقلية.",
        price: "650,000 LBP",
        englishDescription: "Deep Fried Crispy Fish served with Wedges and Tartare.",
        arabicDescription: "سمك مقرمش مقلي يقدم مع ودجز و طرطور.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 18,
        englishTitle: "Crispy Chicken Platter.",
        arabicTitle: "طبق دجاج مقرمش.",
        price: "600,000 LBP",
        englishDescription: "Crispy Chicken Strips, Wedges (Ayoli Sauce).",
        arabicDescription: "شرائح دجاج مقرمشة و ودجز (صلصة أيولي).",
        image: "uploads/arancini.jpg"
      },
      {
        id: 19,
        englishTitle: "Oriental Beef Strips.",
        arabicTitle: "شرائح لحم البقر الشرقية.",
        price: "850,000 LBP",
        englishDescription: "Grilled Beef Strips, served with Thyme, Rosemary and Raspberry/Cherry Vinaigrette.",
        arabicDescription: "شرائح لحم بقري مشوية، تقدم مع الزعتر وإكليل الجبل وصلصة التوت/الكرز.",
        image: "uploads/arancini.jpg"
      }
    ]
  },
  desserts: {
    title: "Desserts | الحلويات",
    description: {
      english: "Fairy Tale Confections with Happy Endings.",
      arabic: "حلويات القصص الخيالية ذات النهاية السعيدة."
    },
    items: [
      {
        id: 13,
        englishTitle: "Grilled Chicken Breast.",
        arabicTitle: "صدور دجاج مشوية.",
        price: "750,000 LBP",
        englishDescription: "Served with Wedges, Creamy Cajen Sauce.",
        arabicDescription: "يُقدم مع ودجز وصلصة كاجين الكريمية.",
        image: "uploads/bruschetta.jpg"
      },
      {
        id: 14,
        englishTitle: "Grilled Beef Strip.",
        arabicTitle: "شرائح لحم البقر المشوية.",
        price: "850,000 LBP",
        englishDescription: "Served with Creamy Pepper Sauce, Wedges.",
        arabicDescription: "تقدم مع صلصة الفلفل الكريمية و ودجز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 15,
        englishTitle: "Sea Food Mix Platter Fish.",
        arabicTitle: "طبق مزيج المأكولات البحرية والأسماك.",
        price: "650,000 LBP",
        englishDescription: "Deep Fried Crispy Shrimps, Fish, Calamari served with Crab Salad and Wedges.",
        arabicDescription: "قريدس مقرمش مقلي، سمك، كاليماري يقدم مع سلطة الكراب و ودجز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 16,
        englishTitle: "Shrimp Platter (Spicey).",
        arabicTitle: "طبق الروبيان (حار).",
        price: "650,000 LBP",
        englishDescription: "Deep Fried Crispy Shrimps, Tartare, served with Wedges.",
        arabicDescription: "قريدس مقرمش مقلي، طرطور، يقدم مع ودجز.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 17,
        englishTitle: "Fish and Chips Platter.",
        arabicTitle: "طبق السمك والبطاطس المقلية.",
        price: "650,000 LBP",
        englishDescription: "Deep Fried Crispy Fish served with Wedges and Tartare.",
        arabicDescription: "سمك مقرمش مقلي يقدم مع ودجز و طرطور.",
        image: "uploads/arancini.jpg"
      },
      {
        id: 18,
        englishTitle: "Crispy Chicken Platter.",
        arabicTitle: "طبق دجاج مقرمش.",
        price: "600,000 LBP",
        englishDescription: "Crispy Chicken Strips, Wedges (Ayoli Sauce).",
        arabicDescription: "شرائح دجاج مقرمشة و ودجز (صلصة أيولي).",
        image: "uploads/arancini.jpg"
      },
      {
        id: 19,
        englishTitle: "Oriental Beef Strips.",
        arabicTitle: "شرائح لحم البقر الشرقية.",
        price: "850,000 LBP",
        englishDescription: "Grilled Beef Strips, served with Thyme, Rosemary and Raspberry/Cherry Vinaigrette.",
        arabicDescription: "شرائح لحم بقري مشوية، تقدم مع الزعتر وإكليل الجبل وصلصة التوت/الكرز.",
        image: "uploads/arancini.jpg"
      }
    ]
  },
  freshjuice: {
    title: "Cold Beverages | مشروبات باردة",
    description: {
      english: "Where science meets sublime flavor.",
      arabic: "حيث يلتقي العلم بالنكهة السامية"
    },
    items: [
      {
        id: 26,
        englishTitle: "Fresh orange.",
        arabicTitle: "عصير برتقال طازج.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 27,
        englishTitle: "Fresh Lemonade.",
        arabicTitle: "عصير الليمون الطازج.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 28,
        englishTitle: "Fresh Mulberry.",
        arabicTitle: "عصير التوت الطازج.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
            {
        id: 30,
        englishTitle: "Fresh Mango.",
        arabicTitle: "عصير المانجا الطازج.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 31,
        englishTitle: "Fresh Strawberry.",
        arabicTitle: "عصير الفراولة الطازج.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 29,
        englishTitle: "Fresh Passion Fruit Lemonade.",
        arabicTitle: "ليموناضة فاكهة الباشن.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 47,
        englishTitle: "Seasonal Fresh Jasmine lemonade.",
        arabicTitle: "عصير ليمون الياسمين الطازج الموسمي.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 33,
        englishTitle: "Seasonal Fresh Water melon juice.",
        arabicTitle: "عصير البطيخ الطازج الموسمي.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 32,
        englishTitle: "minted lemonade (fresh, Frozen).",
        arabicTitle: "عصير ليمون بالنعناع (طازج، مجمد).",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 33,
        englishTitle: "Detox 1.",
        arabicTitle: "عصير البطيخ الطازج الموسمي.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 33,
        englishTitle: "detox 2.",
        arabicTitle: "عصير البطيخ الطازج الموسمي.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
    ]
  },
    milkshake: {
    title: "Milkshake | ميلك شيك",
    description: {
      english: "Creamy, dreamy milkshake delights",
      arabic: "ميلك شيك كريمي ولذيذ"
    },
    items: [
      {
        id: 57,
        englishTitle: "Classic Vanilla Milkshake",
        arabicTitle: "ميلك شيك فانيليا كلاسيكي",
        price: "300,000 LBP",
        englishDescription: "Creamy vanilla milkshake with whipped cream",
        arabicDescription: "ميلك شيك فانيليا كريمي مع كريمة مخفوقة",
        image: "uploads/milkshake.jpg"
      },
      {
        id: 58,
        englishTitle: "Chocolate Dream",
        arabicTitle: "حلم الشوكولاتة",
        price: "300,000 LBP",
        englishDescription: "Rich chocolate milkshake with chocolate syrup",
        arabicDescription: "ميلك شيك شوكولاتة غني مع شراب الشوكولاتة",
        image: "uploads/milkshake.jpg"
      },
      {
        id: 59,
        englishTitle: "Strawberry Fields",
        arabicTitle: "حقول الفراولة",
        price: "300,000 LBP",
        englishDescription: "Fresh strawberry milkshake with real fruit",
        arabicDescription: "ميلك شيك فراولة طازج مع فاكهة حقيقية",
        image: "uploads/milkshake.jpg"
      },
      {
        id: 60,
        englishTitle: "Caramel Crunch",
        arabicTitle: "كرانش كراميل",
        price: "350,000 LBP",
        englishDescription: "Caramel milkshake with crushed cookies",
        arabicDescription: "ميلك شيك كراميل مع بسكويت مفتت",
        image: "uploads/milkshake.jpg"
      }
    ]
  },
  hamsatcocktails: {
    title: "Hamsat Cocktails | كوكتيلات همسات",
    description: {
      english: "Signature cocktails crafted with passion",
      arabic: "كوكتيلات مميزة مصنوعة بشغف"
    },
    items: [
      {
        id: 35,
        englishTitle: "Virgin Pina Colada.",
        arabicTitle: "فيرجن بينا كولادا.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 36,
        englishTitle: "Strawberry Pina Colada.",
        arabicTitle: "بينا كولادا الفراولة.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 37,
        englishTitle: "Virgin Mohitto.",
        arabicTitle: "فيرجن موهيتو.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 45,
        englishTitle: "Jamaica.",
        arabicTitle: "جامايكا.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 46,
        englishTitle: "Ginger Ale Mix (7up Lime Syrup).",
        arabicTitle: "مزيج الزنجبيل (شراب الليمون 7up).",
        price: "150,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
    ]
  },
  coffeedrinks: {
    title: "Hot Beverages | مشروبات ساخنة",
    description: {
      english: "A brew that sees your future.",
      arabic: "مشروب يرى مستقبلك."
    },
    items: [
      {
        id: 38,
        englishTitle: "Espresso.",
        arabicTitle: "اسبريسو.",
        price: "150,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 39,
        englishTitle: "Dopio (double Espresso).",
        arabicTitle: "دوبيو (دوبل إسبريسو).",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 40,
        englishTitle: "Cuppocino.",
        arabicTitle: "كبوتشينو.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 41,
        englishTitle: "Cafe Late (Carameal Dark Chocolate).",
        arabicTitle: "كافيه لاتيه (شوكولاتة داكنة بالكراميل).",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 42,
        englishTitle: "Iced Cafe Late (Carameal Dark Chocolate).",
        arabicTitle: "آيس كافيه لاتيه (شوكولاتة داكنة بالكراميل).",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 43,
        englishTitle: "Hot Chocolate (Iced Hot Chocolate).",
        arabicTitle: "شوكولاتة ساخنة (شوكولاتة ساخنة مثلجة).",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 44,
        englishTitle: "Fresh Jasmin Tea Seasonal.",
        arabicTitle: "شاي الياسمين الطازج الموسمي.",
        price: "150,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      }
    ]
  },
  softdrinks: {
    title: "Soft Drinks | المشروبات الغازية",
    description: {
      english: "Precision-engineered refreshment.",
      arabic: "انتعاش مصمم بدقة."
    },
    items: [
      {
        id: 48,
        englishTitle: "Pepsi.",
        arabicTitle: "بيبسي.",
        price: "100,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 49,
        englishTitle: "Diet Pepsi.",
        arabicTitle: "دايت بيبسي.",
        price: "100,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 50,
        englishTitle: "7UP.",
        arabicTitle: "7 اب.",
        price: "100,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 51,
        englishTitle: "Diet 7UP.",
        arabicTitle: "دايت 7 اب.",
        price: "100,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 52,
        englishTitle: "Energy Drink.",
        arabicTitle: "مشروب طاقة.",
        price: "200,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 53,
        englishTitle: "Water large.",
        arabicTitle: "مياه كبيرة.",
        price: "100,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 54,
        englishTitle: "Water Small.",
        arabicTitle: "مياه صغيرة.",
        price: "50,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 55,
        englishTitle: "Sparkling Water large.",
        arabicTitle: "مياه غازية كبيرة.",
        price: "250,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 56,
        englishTitle: "Sparkling Water Small.",
        arabicTitle: "مياه غازية صغيرة.",
        price: "150,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      }
    ]
  },
  shisha: {
    title: "ٍShisha | اراكيل",
    description: {
      english: "Rich clouds of aged tobacco, sweetened to perfection.",
      arabic: "سحب غنية من التبغ القديم، محلاة إلى حد الكمال."
    },
    items: [
      {
        id: 45,
        englishTitle: "",
        arabicTitle: "اركيلة عجمي",
        price: "450,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 45,
        englishTitle: "",
        arabicTitle: "اركيلة تفاحتين",
        price: "450,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 45,
        englishTitle: "",
        arabicTitle: "اركيلة حامض و نعناع",
        price: "450,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 45,
        englishTitle: "",
        arabicTitle: "اركيلة علكة",
        price: "450,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 45,
        englishTitle: "",
        arabicTitle: "اركيلة حب",
        price: "450,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
      {
        id: 45,
        englishTitle: "",
        arabicTitle: "اركيلة همسات سبيشيل",
        price: "700,000 LBP",
        englishDescription: "",
        arabicDescription: "",
        image: "uploads/souffle.jpg"
      },
    ]
  },
};